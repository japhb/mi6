...
unit class App::Mi6:ver<1.1.4>:auth<cpan:SKAJI>;
...
multi method cmd('new', $module is copy, :$zef) {
    ...
    if $zef {
        try {
            require ::("App::Mi6::Fez");
            my $u = ::("App::Mi6::Fez").username;
            if $u {
                $auth = "zef:$u";
            } else {
                die;
            }
        } or die "To create a distribution for Zef ecosystem,\n"
...
multi method cmd('release', Bool :$keep, Str :$next-version, Bool :$yes) {
    ...
    my $upload-class = config("UploadToZef") ?? "UploadToZef" !! "UploadToCPAN";
